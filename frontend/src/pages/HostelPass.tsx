...
const HostelPass = () => {
  ...
  return (
    <div className="h-full bg-slate-50 flex flex-col font-sans animate-fade-in relative pb-24">
      ...
      <div className="bg-app-dark p-6 pt-12 shadow-md relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/campus')} className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors">
          ...
          <div className="flex flex-col gap-4">
            {passes.map(p => (
              <div key={p.id} className="bg-white rounded-[24px] p-5 shadow-sm border border-slate-100 flex flex-col relative overflow-hidden animate-slide-up">
                ...
                <h3 className="text-base font-bold text-slate-900 pr-24 leading-tight mb-3">{p.reason}</h3>
                ...
                {p.status === 'approved' && p.qr_code_url && (
                  <button 
                    onClick={() => setSelectedQR(p.qr_code_url)}
      ...
      {/* QR Code Modal */}
      {selectedQR && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-md animate-fade-in p-6" onClick={() => setSelectedQR(null)}>
          <div className="bg-white rounded-[32px] p-8 shadow-2xl animate-scale-in flex flex-col items-center max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <h2 className="text-2xl font-black text-slate-900 mb-2">Gate Pass QR</h2>
            <p className="text-sm font-semibold text-slate-500 mb-8 text-center">Scan this at the main security gate to exit.</p>
            
            <div className="bg-white p-4 rounded-2xl shadow-inner border border-slate-100 mb-8">
              <img src={selectedQR} alt="Gate Pass QR" className="w-48 h-48" />
            </div>
            
            <button onClick={() => setSelectedQR(null)} className="w-full bg-slate-100 text-slate-800 py-4 rounded-[20px] text-sm font-bold hover:bg-slate-200 transition-colors">
              Close
            </button>
          </div>
        </div>
