...
import { ChevronLeft, Award, Star, Zap, Code, Users, Trophy, BookOpen, Share2, Info, Plus, CheckCircle } from 'lucide-react';
...
interface Holder { student_id:string; name:string; department:string; earned_at:string; }

const iconMap: Record<string,React.ReactNode> = {
  award:<Award className="w-8 h-8"/>, star:<Star className="w-8 h-8"/>, zap:<Zap className="w-8 h-8"/>,
  code:<Code className="w-8 h-8"/>, users:<Users className="w-8 h-8"/>, trophy:<Trophy className="w-8 h-8"/>,
...
const SkillBadges = () => {
  ...
  const openBadgeModal = async (badge: Badge, earnedObj: Earned|null = null) => {
    ...
    setHolders([]);
    try {
      const { data } = await api.get(`/career/badges/${badge.id}/holders`);
      ...
      <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 pt-12 shadow-lg relative overflow-hidden">
        ...
        <div className="flex items-center justify-between relative z-10 mb-4">
          <div className="flex items-center gap-3">
            <button onClick={()=>nav('/career')} className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 transition-colors"><ChevronLeft className="w-5 h-5 text-white"/></button>
            <div><h1 className="text-xl font-bold text-white">Skill Badges</h1><p className="text-xs text-indigo-200">Verifiable Credentials</p></div>
          </div>
        </div>
        {/* Stats */}
        ...
        : tab==='earned' ? (
          earned.length===0 ? (
            <div className="text-center py-20 px-6 animate-fade-in">
              <div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-white shadow-sm"><Trophy className="w-8 h-8 text-indigo-300"/></div>
                ...
                return (
                  <button key={e.id || idx} onClick={() => openBadgeModal(b, e)} className="group bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex flex-col items-center text-center hover:shadow-xl hover:-translate-y-1 transition-all duration-300" style={{ animationDelay: `${idx * 50}ms` }}>
                    <div className={`relative w-20 h-20 rounded-[28px] bg-gradient-to-br ${grad} flex items-center justify-center text-white shadow-lg mb-4 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>
                      ...
                      {/* Sparkles */}
                      <div className="absolute -top-1 -right-1 w-3 h-3 bg-yellow-300 rounded-full animate-ping opacity-0 group-hover:opacity-100"></div>
                    </div>
      ...
      {selectedBadge && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-md animate-fade-in p-4" onClick={() => setSelectedBadge(null)}>
          <div className="bg-white rounded-[32px] w-full max-w-sm shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]" onClick={e=>e.stopPropagation()}>
            {/* Modal Header/Banner */}
            <div className={`relative pt-12 pb-8 px-6 bg-gradient-to-br ${catColors[selectedBadge.badge.category] || 'from-slate-500 to-slate-600'} text-center overflow-hidden`}>
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              ...
              <div>
                ...
                {loadingHolders ? (
                  ...
                  <p className="text-xs text-slate-400 italic">No one else has earned this yet. Be the first!</p>
                )}
              </div>
            </div>
