              ...
              return (
                <button key={b?.id || Math.random().toString()} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} onClick={() => openBadgeModal(b, e)} className={`text-left rounded-3xl p-4 shadow-sm border flex items-start gap-4 transition-all hover:shadow-md ${isEarned ? 'bg-indigo-50/30 border-indigo-100' : 'bg-white border-slate-100 hover:border-indigo-200'}`}>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-start">
                    ...
                    {b.description && <p className="text-xs text-slate-500 line-clamp-2 leading-snug mt-2">{b.description}</p>}
                  </div>
          ...
          <div className="bg-white rounded-[32px] w-full max-w-sm shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="mb-6">
                ...
                <p className="text-sm font-medium text-slate-700 leading-relaxed mt-2">{selectedBadge.badge.description}</p>
              </div>
